...
fprintf('\nUniform plane boundaries (Stage 3B):\n');
for i = 1:num_planes
...
Ns = 120;        % samples per dimension (memory-safe)
max_iter = 15;  % IRLS iterations
eps = 1e-6;
...
for i = 1:num_planes
    ...
    % Regression model: f ≈ pm*m + pa*a + b
    ...
    % IRLS loop (L1 approximation, memory-safe)
    for k = 1:max_iter
        r = y - X*theta;
        w = 1 ./ (abs(r) + eps);    % L1 weights

        Xw = X .* w;                % row-wise weighting
        theta = (X' * Xw) \ (Xw' * y);
    end
...
title('Approximate Surface (Uniform + IRLS)');
xlabel('m'); ylabel('a'); zlabel('f');
...
fprintf('\nTable-I Coefficients (Stage 3B – Uniform + IRLS)\n');
